...
class GlobalDataModule(pl.LightningDataModule):
    def __init__(
        ...
        self.topk_pretrain = topk_pretrain
        self.topk_classify = topk_classify
    ...
    def contrast_train_dataloader(self):
        return DataLoader(
            ...
            shuffle=False,
            collate_fn=lambda batch: self._contrastive_collate(batch, topk=self.topk_pretrain),
            num_workers=8
        )
    ...
    def train_dataloader(self):
        return DataLoader(
            ...
            shuffle=False,
            collate_fn=lambda batch: self._classification_collate(batch, topk=self.topk_classify),
            num_workers=8
        )
    ...
    def test_dataloader(self):
        return DataLoader(
            self.global_test,
            batch_size=self.batch_size,
            collate_fn=lambda batch: self._classification_collate(batch, topk=self.topk_test),
            num_workers=8
        )

    def _classification_collate(self, batch, topk=None):
        try:
            return self._build_contrastive_batch(batch, topk=topk)
        except Exception as e:
            print(f"Collate error: {str(e)}")
            torch.save(batch, "error_batch.pt")
            raise

    def _contrastive_collate(self, batch, topk=None):
        # Original graph structure batch construction
        batch_data = self._build_contrastive_batch(batch, topk=topk if topk is not None else self.topk_pretrain)
    ...
    def _build_contrastive_batch(self, samples, topk=None):
        max_regions_in_file = self.max_regions
        use_k = max_regions_in_file if (topk is None) else min(topk, max_regions_in_file)
        d = self.emb_dim

        all_region_edges = []
        all_semantic_embs = []
        all_structural_embs = []
        region_code_embs_lists = []
        ...
        for data in samples:
            ...
            scores_full = data.region_score.view(-1)  # (R,)
            R = scores_full.numel()
            k = R if topk is None else min(use_k, R)
            topk_idx = torch.arange(k, dtype=torch.long)

            sel_node_index_lists = [data.node_index_lists[i] for i in topk_idx]
            sel_region_line_numbers = [data.region_line_numbers_lists[i] for i in topk_idx]
            sel_region_scores = scores_full[topk_idx]
            sel_region_features = [data.region_features[i] for i in topk_idx]
            sel_region_edges = [data.region_edges[i] for i in topk_idx]

            sel_region_semantic_emb = data.region_semantic_emb[topk_idx]  # (k, d)
            sel_region_structural_emb = data.region_structural_emb[topk_idx]  # (k, d)

            sel_region_code_embeddings = [data.region_code_embeddings_lists[i] for i in topk_idx]

            region_features_pack = self._process_regions(
                sel_node_index_lists,
                sel_region_line_numbers,
                sel_region_scores,
                sel_region_features,
                node_offset,
                use_k,
            ...
            for kname, v in region_features_pack.items():
                region_data[kname].append(v)
            ...
            all_region_edges.append(sel_region_edges)
            all_semantic_embs.append(sel_region_semantic_emb)
            all_structural_embs.append(sel_region_structural_emb)
            region_code_embs_lists.append(sel_region_code_embeddings)

        region_emb_data = self._process_region_embeddings(region_code_embs_lists, target_regions=use_k)
        global_emb_data = self._process_global_embeddings([data.global_code_embedding for data in samples])

        region_edges = self._process_region_edges(all_region_edges, use_k, max_region_nodes)
        region_semantic_emb = self._process_region_embs(all_semantic_embs, use_k, d)
        region_structural_emb = self._process_region_embs(all_structural_embs, use_k, d)

        region_features = torch.stack(region_data['features'])   # (batch, use_k, max_nodes, d)
    ...
    def _process_region_embeddings(self, all_region_embeddings, target_regions=None):
        """Region embedding alignment"""
        # Calculate the maximum dimension
        max_regions = self.max_regions if target_regions is None else int(target_regions)
        max_nodes = max(
            emb.size(0)
