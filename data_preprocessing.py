...
class DatasetBuilder:
    ...
    def build_classify_dataset(self):
        ...
        for c_file in pbar:
            try:
                ...
                error_msg = f"[{c_file} joern extraction error, please check source code integrity.]"
    ...
    def _save_error_log(self, error_log: List[str]):
        if error_log:
            with open("data_preprocess_errors.log", "w") as f:
                f.write("\n".join(error_log))
    ...
    def filter_common_patterns(self, nodes, threshold=0.05):
        """Filter high-frequency general structures (such as simple if judgment)"""
...
if __name__ == "__main__":
    # Configuration corresponding to the three data sets
    DATASET_CONFIGS = {
        "FFMPeg+Qemu": {
            "c_dir":        "./dataset/FFMPeg+Qemu/c/",
            "js_dir":       "./dataset/FFMPeg+Qemu/js/",
            "vul_lines":    "./dataset/FFMPeg+Qemu/vulnerable_lines.json",
            "w2v_path":     "./dataset/FFMPeg+Qemu/W2V/Devign-128-20.wordvectors"
        },
        "BigVul": {
    ...
    parser = argparse.ArgumentParser(description="Preprocess one of the three datasets")
    parser.add_argument(
